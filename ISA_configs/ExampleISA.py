...
CPU_stats = {
  ...
  # Stack information
  "SP_IS_REG": True,             # Is the stack pointer stored in a register or a memory location? (Recommended is True)
  "SP_LOCATION": 255, # If you want to, you can specify a custom location for the stack pointer, if it isn't a register. (Recommended is at the end of RAM)
  "FLIPPED_STACK": False,        # Should the stack grow upwards instead of downwards? (Recommended is False)
  "STACK_START": 0, # Additionally, where should the stack start? (Default is 0, pointing to the end of RAM)
  "MANUAL_SP_REMOVAL": False,    # Should "SP" be expanded or should it be left as "SP" in the final code? (Helps to parse out STRs and LODs using the SP)
  }
...
# This calculates the length of your program, and adjusts the #X or MX memory values accordingly.
# You will need to tweak this if your instructions are multi-word. See the RemoveLabels() example above.
def getProgramLength(code):
    if not CPU_stats["RUN_RAM"]:
        return 0
    length = 0
    for line in code:
        length += 1
